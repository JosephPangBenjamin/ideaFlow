...
function StatCard({ title, value, icon, gradient, delay = 0 }: StatCardProps) {
    return (
        ...
        >
            {/* Dynamic Glow Effect */}
            <div className={`absolute -top-10 -right-10 w-32 h-32 rounded-full opacity-20 blur-2xl ${gradient.replace('bg-', 'bg-')}`} />
        ...
        >
            ...
            <div className="mb-10 flex items-end justify-between">
                <motion.div variants={fadeInUp}>
                    <h1 className="text-4xl font-heading font-bold text-white tracking-tight">下午好, User</h1>
                    <p className="text-slate-400 mt-2 text-lg">今天阳光明媚，适合捕捉灵感。</p>
                </motion.div>

                {/* Weather/Context Widget (Placeholder) */}
                <motion.div variants={fadeInUp} className="hidden md:flex items-center gap-4 text-slate-300 bg-white/5 px-4 py-2 rounded-full border border-white/5 backdrop-blur-sm">
                    <span>🌱 创意生长季</span>
            ...
            <div className={`grid gap-6 transition-all duration-500 ease-spring ${isSidebarOpen
            ...
            <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex gap-2">
                            {['周', '月', '年'].map(t => (
                                <button key={t} className="px-3 py-1 text-sm rounded-lg text-slate-400 hover:bg-white/5 hover:text-white transition-colors">{t}</button>
                            ))}
                        </div>
                    </div>
                ...
                >
                    ...
                    <div className="space-y-4">
                        <QuickActionRow icon={<IconBulb />} label="记录新想法" shortcut="N" color="text-blue-400" />
                        <QuickActionRow icon={<IconCheckCircle />} label="创建任务" shortcut="T" color="text-emerald-400" />
                        <QuickActionRow icon={<IconApps />} label="开启画布" shortcut="C" color="text-purple-400" />
                    </div>
                </motion.div>
            </div>
        </motion.div>
...
function QuickActionRow({ icon, label, shortcut, color }: { icon: any, label: string, shortcut: string, color: string }) {
    return (
        <div className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 cursor-pointer transition-colors group">
            ...
            <kbd className="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-xs text-slate-500 font-mono group-hover:text-slate-300 transition-colors">⌘{shortcut}</kbd>
        </div>
    )
}
